...
class __TwigTemplate_1540672e5d5bebe096a1c6a43dcf76f7 extends \Twig\Template
{
    private $source;
    private $macros = [];
...
<ul {{ attributes.addClass(menu_classes) }} role=\"menu\">
    ...
    {% for item in items %}
      ...
      <li{{ item.attributes.addClass(item_classes) }} role=\"none\">
        ...
        {% if (item.url|render starts with '/') or (item.url|render starts with 'http')  %}
          <a href=\"{{ item.url }}\" class=\"w3-button link-enabled clickable-dropdown-menu-link toggle-parent\" role=\"menuitem\" aria-haspopup=\"true\">{{ item.title }}</a>
          <span class=\"dropdown-arrow w3-hover-over clickable-dropdown-menu-fa-down\"><i class=\"fa fa-chevron-down\"></i></span>
        {% else %}
      ...
      <li{{ item.attributes.addClass(item_classes) }} role=\"none\">
        ...
        {% if (item.url|render starts with '/') or (item.url|render starts with 'http')  %}
          <a href=\"{{ item.url }}\" class=\"w3-button link-enabled clickable-dropdown-menu-link toggle-child\" role=\"menuitem\" aria-haspopup=\"true\">{{ item.title }}</a>
          <span class=\"right-arrow w3-hover-over clickable-dropdown-menu-fa-right\"><i class=\"fa fa-chevron-right\"></i></span>
          <span class=\"dropdown-arrow w3-hover-over clickable-dropdown-menu-fa-down\"><i class=\"fa fa-chevron-down\"></i></span>
        {% else %}
  ...
  {% endif %}
  {% endmacro %}
", "themes/contrib/d8w3css/templates/navigation/menu.html.twig", "/var/www/html/web/themes/contrib/d8w3css/templates/navigation/menu.html.twig");
    }
